...
async function getPartner(id: string): Promise<PartnerWithUsers | null> {
  ...
  const usersRes = await fetch(`/api/admin/users?partner_id=${id}`);
...
export default function PartnerDetailsPage({ params }: { params: { id: string } | Promise<{ id: string }> }) {
  // Next.js 14+ unwrap params if it's a Promise
  // @ts-expect-error Next.js 14 dynamic params may be a Promise, so we unwrap with React.use()
  const actualParams = typeof params.then === 'function' ? React.use(params) : params;
  const id = actualParams && 'id' in actualParams ? actualParams.id : undefined;
  ...
  // Só busca o parceiro quando os segmentos estiverem carregados
  useEffect(() => {
    if (id && marketSegmentsLoaded) {
      fetchPartner();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [id, marketSegmentsLoaded]);
