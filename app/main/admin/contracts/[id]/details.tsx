...
export default function ProjectDetailsTab({ project, editMode, setEditMode }: ProjectDetailsTabProps) {
  ...
  // SLA Dialog state
  const slaFeatureTagEnabled = typeof process !== 'undefined' && (process.env.NEXT_PUBLIC_SLA_FEATURE_TAG === 'true' || process.env.NEXT_PUBLIC_SLA_FEATURE_TAG === 'True');
  ...
  return (
    <TooltipProvider>
      ...
      {slaFeatureTagEnabled && <Card className="mt-4">
        <div className="px-6 pb-4 pt-2">
          <h2 className="flex items-center text-lg font-semibold">
